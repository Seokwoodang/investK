  ...
  try {
    ...
    const msg = await client.messages.create({
      ...
      system:
        ...
        (focus ? `${focus} ` : '') +
        // why: 제목을 그대로 반복하지 말 것. 제목엔 없는 '투자 함의'를 담는다 — 이 뉴스가 해당 종목·섹터에 왜/어떻게 작용하는지(메커니즘), 무엇을 지켜봐야 하는지.
        'why는 제목의 재진술이 아니라 "투자 관점의 함의"여야 한다. 제목에 이미 있는 사실을 그대로 옮기지 말고, 그 사실이 target의 실적·수급·밸류에이션에 어떻게 연결되는지 또는 다음에 주목할 포인트를 한 줄로 쓴다. ' +
        '반드시 JSON 배열만 출력한다(설명·코드펜스 금지). 형식: [{"i":후보번호,"target":"영향 대상","impact":"호재|악재|중립","importance":"상|중|하","why":"제목엔 없는 투자 함의 한 줄","title_ko":"한국어 제목 또는 빈 문자열"}]. ' +
